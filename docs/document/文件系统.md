...
BirdOS基于XV6实现，文件系统采用类ext4的方式设计，文档分为两部分：xv6的基本功能与我们改进与新增的功能。
...
`/user/test/bigfiletest` 用于测试该功能，该程序希望能够创建一个包含65803个块的大文件并验证读写操作，当块的数量 `blocks` 达到 100 的倍数时，程序会在控制台打印一个 `.`，以显示写入进度。若没有二级间接块，是不支持这种大文件的读写的。
