...
int thread_create(void(*start_routine)(void*),void*arg)
{
    ...
    void* stack=malloc(4096);    // 为用户栈分配一页
    int pid=clone((uint64)start_routine,(uint64)arg,(uint64)stack);
...
int thread_join(void)
{
    for(int i=0;i<NTHREAD;i++)
    {
        if(threads[i].used==1)
        {
            ...
            int pid=join((uint64)&threads[i].ustack);
            if(pid>0)
            {
                remove_thread(&pid);
                return pid;
